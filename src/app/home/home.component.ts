...
export class HomeComponent implements OnInit {
    ...
    private filterReports (dataSet: Array<Object>) {
        this.clearReports ();
        let conditions = this.getConditionArray ();

        for (let index in dataSet) {
            let report = dataSet[index];
            let sent = new Date(report['send_time']);
        ...
        for (let i = 0; i < 5; i++) {
            if (this.statistics[i]['numberOf'] > 0) {
                ...
                this.total.bounceRate += this.statistics[i]['bounceRate'] = this.statistics[i]['bounceRate'] / this.statistics[i]['numberOf'];
            }
        }
    ...
    private getReportData (args) {
        let self = args || this;
        self.componentService.getData()
            .then(data => {
                self.filterReports (data);
            })
            .catch(error => {
                self.errorService.showError(error);
            });
    }
